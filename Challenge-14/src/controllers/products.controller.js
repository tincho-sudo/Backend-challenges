...
const newProduct = async (req, res) => {
  try {
    ...
    const addedProduct = await productsService.newProduct(product);
...
const updateProduct = async (req, res) => {
  try {
    ...
    if (user.isPremium && product.owner !== user.email) {
      return res.status(403).json({ success: false, error: "No hay permisos para actualizar este producto" });
    }
...
const deleteProduct = async (req, res) => {
  try {
    ...
    if (user.isPremium && product.owner !== user.email) {
      return res.status(403).json({ success: false, error: "No hay permisos para eliminar este producto" });
    }
