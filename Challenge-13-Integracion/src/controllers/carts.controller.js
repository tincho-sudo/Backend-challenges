  ...
  try {
    ...
    if (user.role === "user") {
      ...
      if (!product || product.stock === 0) {
        return res.status(404).json({ success: false, error: "Producto no encontrado o fuera de stock" });
      }

      if (user.isPremium && product.owner === userId) {
        return res.status(403).json({ success: false, error: "No es posible permitido agregar tu propio producto al carrito" });
      }
